...
# ╔═╡ b33847e0-8886-11ef-3e75-4771ed679cda
using DifferentialEquations

# ╔═╡ 062e7a3d-8275-45fb-9a15-0b4b03ce3f86
using Plots

# ╔═╡ 2fe51bf8-cae0-4562-b8ad-553c580a61a1
function pendulum!(du, u, p, t)
	L, g = p
	θ, ω = u
	du[1] = ω
	du[2] = -g/L * sin(θ)
	# here ! allows us to modify given arguments (here, du) in-place
end

# ╔═╡ bc4732cb-bae7-4adb-a575-0ba0625369bd
p = [1.0, 9.8]
# L = 1 and g = 9.8

# ╔═╡ 1eb991cb-17b8-40ee-aa9b-2eb3eeefff20
begin
	angles = [5,90,175]
	# assign initial angles to an array to work with it later
	u0_list = []
	for angle in angles
		u0 = [deg2rad(angle), 0]
		# θ, ω: initial angle and initial angular velocity <- initial conditions
		# initial angle = deg2rad(angle) and initial velocity = 0
		push!(u0_list, u0)
	end
end

# ╔═╡ 625c8956-15ec-46d0-9e9c-49e4e9a0b960
tspan = (0, 10)
# the time interval over which we want to solve the DE

# ╔═╡ 59ac20b8-d039-4937-85e6-0becfe414fbf
solution = []

# ╔═╡ e3d93b44-6539-4b04-aaf9-1b693c2170f4
for ic in u0_list
		ode = ODEProblem(pendulum!, ic, tspan, p)
		# DifferentialEquations : ODEProblem() defines an ODE
		sol5d = solve(ode, Tsit5())
		# DifferentialEquations : solve() solves a mathematical problem
		push!(solution, sol5d)
		# we collect the solutions for each ic in solution
end

# ╔═╡ 8643e1f7-5f47-4891-9c31-b7dc3b39e87b


# ╔═╡ 1abcdb0c-3c46-4bfc-bbec-5c472746f2d5
begin
plot(solution[1]; idxs=1, lw=4, lc=:green, label="θ₀=5°",
legend=:outerright, title="Simple Gravity Pendulum")
plot!(solution[2]; idxs=1, lw=4, lc=:blue, label="θ₀=90°",
legend=:outerright)
plot!(solution[3]; idxs=1, lw=4, lc=:red, label="θ₀=175°",
legend=:outerright)
end
# we plot ic solutions in the same display
...
manifest_format = "2.0"
project_hash = "1281049eb106e105d6586a2c7d7f78368278b9a2"
