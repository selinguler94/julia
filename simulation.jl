...
# ╔═╡ 06fe6d20-8885-11ef-39dc-231f4fa6c1ad
using DifferentialEquations

# ╔═╡ a6a4ec4b-6e79-4841-9e85-20663afe0673
using Plots

# ╔═╡ 1103ba20-c859-4f7d-8ae4-6c18ee18077e
...
# ╔═╡ 59435c13-8cc5-4fa3-855f-b2c362ef2afe
p = [1.0, 9.8]

# ╔═╡ 4206d8a9-ea7c-46b5-93bd-b689e3a24de6
begin
	angles = [5,90,175]
	u0_list = []
	for angle in angles
...
# ╔═╡ b1f10b06-a393-4d70-b97a-e71245dc5919
tspan = (0, 10)

# ╔═╡ 3320189c-afab-410c-8ec8-339c51f31da0
solution = []

# ╔═╡ a76f7029-524b-4342-9ec5-6960818558b9
for angle in u0_list
	ode = ODEProblem(pendulum!, angle, tspan, p)
	sol5d = solve(ode)
	push!(solution, sol5d)
end

# ╔═╡ 4beb6800-a886-4253-942f-469f50bac6c7
print(solution)

# ╔═╡ da0d2798-0b90-40db-9f47-29fac451b5da
begin
...
# ╔═╡ 925e5f0b-ad83-4895-85a5-d01e306bb5e4
begin
	
	theta = solution[1][1, :] # Angle values for simulation (framing)

	
	x = p[1] * sin.(theta)
	y = -p[1] * cos.(theta)
	
	# here, the difference between sin() and sin.() is that sin.() doesn't take a single value but sin.() does such as arrays.
	
	animation = @animate for i in eachindex(theta)
		
		plot([0, x[i]], [0, y[i]], lw=2, legend=false, linecolor=:black, xlims=(-1.2, 1.2), ylims=(-1.2, 0.2))
				
	    scatter!([x[i]], [y[i]], color=:black, markersize=20)
	end
	
	gif(animation, "simple_pendulum.gif", fps=10)
